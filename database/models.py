...
class CfgGroup(Base):
    ...
    # tid: Mapped[int] = mapped_column(primary_key=True, autoincrement=True, index=True)
    cfg_fk: Mapped[int] = mapped_column(ForeignKey('configurations.tid'), primary_key=True)
    group_fk: Mapped[int] = mapped_column(ForeignKey('groups.tid'), primary_key=True)
